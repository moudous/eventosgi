...
@php
    ...
    $retornoPorAtividade = request()->routeIs('atividades.recebimentos.show');
    $permissaoVoltar = $retornoPorAtividade ? 'atividades.recebimentoslistar' : 'recebimentos.listar';
    $urlVoltar = $retornoPorAtividade
        ? route('atividades.recebimentos.index', request()->route('atividade'))
        : route('recebimentos.index');
@endphp
<div class="mb-4 d-flex flex-wrap justify-content-between align-items-start gap-3"><div><h1 class="page-title">Recebimento PIX #{{ $recebimento->id }}</h1><p class="page-description mb-0">{{ $atividade?->nome ?? 'Atividade não encontrada' }}</p></div>@if(app(\App\Services\GiPermissionService::class)->permite($permissaoVoltar))<a href="{{ $urlVoltar }}" class="btn btn-outline-secondary">Voltar</a>@endif</div>
