...
@else
 ...
 @if($permissoes->permite('atividades.visualizar_formulario'))<a href="{{ route('inscricoes.publica', ['atividade' => $atividade->hash_publica]) }}" target="_blank" rel="noopener" class="btn btn-sm btn-outline-info listagem-acao" title="Visualizar formulário público" aria-label="Visualizar o formulário público de {{ $atividade->nome }}"><i class="bi bi-card-text"></i></a>@endif
 ...
 @if($permissoes->permite('atividades.wordpress'))<button type="button" class="btn btn-sm btn-outline-dark listagem-acao" title="Copiar shortcode do WordPress" data-shortcode='[eventosgi_formulario id="{{ $atividade->id }}"]'><i class="bi bi-wordpress"></i></button>@endif
