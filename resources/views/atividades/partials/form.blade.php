...
<div class="col-12">
    ...
    <div class="form-check">
        <input class="form-check-input" type="checkbox" id="mostrar_link_evento" name="mostrar_link_evento" value="1" @checked(old('mostrar_link_evento', $atividade?->mostrar_link_evento ?? false))>
...
<div class="col-12 col-md-6" id="campoLinkTransmissao" hidden><div class="d-flex align-items-center justify-content-between gap-3 mb-2"><label class="form-label fw-semibold mb-0" for="link_transmissao">Link de Transmissão</label><select class="form-select form-select-sm w-auto @error('tipo_link_transmissao') is-invalid @enderror" id="tipo_link_transmissao" name="tipo_link_transmissao" aria-label="Formato do link de transmissão" disabled><option value="nao_usar" @selected(old('tipo_link_transmissao', $atividade?->tipo_link_transmissao ?? 'nao_usar') === 'nao_usar')>Não usar link</option><option value="link" @selected(old('tipo_link_transmissao', $atividade?->tipo_link_transmissao) === 'link')>Somente o link</option><option value="iframe" @selected(old('tipo_link_transmissao', $atividade?->tipo_link_transmissao) === 'iframe')>Iframe</option></select></div>@error('tipo_link_transmissao')<div class="invalid-feedback d-block">{{$message}}</div>@enderror<input type="text" class="form-control @error('link_transmissao') is-invalid @enderror" id="link_transmissao" name="link_transmissao" maxlength="10000" value="{{ old('link_transmissao', $atividade?->link_transmissao) }}" placeholder="https://www.youtube.com/watch?v=..." disabled>@error('link_transmissao')<div class="invalid-feedback">{{$message}}</div>@enderror<textarea class="form-control font-monospace bg-dark text-light" id="iframe_transmissao" name="link_transmissao" rows="7" maxlength="10000" spellcheck="false" placeholder="&lt;iframe src=&quot;...&quot;&gt;&lt;/iframe&gt;" disabled>{{ old('link_transmissao', $atividade?->link_transmissao) }}</textarea><div class="form-text" id="ajudaLinkTransmissao"></div></div>
<div class="col-12 col-md-6" data-campo-atividade @if($tipoAtividade === 'somente_inscricao') hidden @endif><label class="form-label fw-semibold" for="local">Local</label><input type="text" class="form-control @error('local') is-invalid @enderror" id="local" name="local" maxlength="255" value="{{ old('local', $atividade?->local) }}" placeholder="Ex.: Laboratório de Endodontia" @disabled($tipoAtividade === 'somente_inscricao')>@error('local')<div class="invalid-feedback">{{$message}}</div>@enderror</div>
...
const modalidade=document.getElementById('modalidade'),tipoLink=document.getElementById('tipo_link_transmissao'),campoLink=document.getElementById('campoLinkTransmissao'),link=document.getElementById('link_transmissao'),iframe=document.getElementById('iframe_transmissao'),ajuda=document.getElementById('ajudaLinkTransmissao');
function atualizarTransmissao(){if(!modalidade||!tipoLink)return;const mostrar=modalidade.value==='ead';campoLink.hidden=!mostrar;tipoLink.disabled=!mostrar;const naoUsar=tipoLink.value==='nao_usar',ehIframe=tipoLink.value==='iframe';link.hidden=naoUsar||ehIframe;iframe.hidden=naoUsar||!ehIframe;link.disabled=!mostrar||naoUsar||ehIframe;iframe.disabled=!mostrar||naoUsar||!ehIframe;ajuda.textContent=naoUsar?'Nenhum link ou iframe será mostrado na página do evento.':(ehIframe?'Cole somente um iframe com endereço HTTP ou HTTPS.':'Informe o link do vídeo. Links do YouTube mostram a miniatura no card.')}
modalidade?.addEventListener('change',atualizarTransmissao);tipoLink?.addEventListener('change',atualizarTransmissao);atualizarTransmissao();
