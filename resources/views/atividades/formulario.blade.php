...
<div class="card content-card mb-4"><div class="card-header"><h2 class="h5 fw-bold mb-0">Pagamento de Inscrição</h2></div><div class="card-body"><div class="row g-3">
<div class="col-12"><div class="form-check"><input class="form-check-input" type="checkbox" id="pagamento_pix_sicoob" @disabled(!$permissoes->permite('atividades.pix'))><label class="form-check-label fw-semibold" for="pagamento_pix_sicoob">PIX Sicoob</label></div><div class="form-text">Gera uma única cobrança com a soma dos valores associados aos campos e itens escolhidos.</div></div>
<div class="col-md-4 pagamento-pix-config"><label class="form-label" for="pagamento_pix_expiracao">Expiração (segundos)</label><input class="form-control" id="pagamento_pix_expiracao" type="number" min="300" max="86400" step="1" value="3600" @disabled(!$permissoes->permite('atividades.pix'))></div>
<div class="col-md-8 pagamento-pix-config"><label class="form-label" for="pagamento_pix_descricao">Descrição do PIX</label><input class="form-control" id="pagamento_pix_descricao" maxlength="140" value="{{ $atividade->nome }}" @disabled(!$permissoes->permite('atividades.pix'))></div>
@if(!$permissoes->permite('atividades.pix'))<div class="col-12"><div class="alert alert-light border mb-0"><i class="bi bi-lock me-1"></i>Seu perfil não possui a permissão <code>atividades.pix</code> para alterar esta configuração.</div></div>@endif
</div></div></div>
...
const pagamentoLegado=(initial.campos||[]).find(campo=>campo.tipo==='pagamento_pix');
const pagamentoInicial=initial.pagamento_inscricao||{pix_sicoob:!!pagamentoLegado,expiracao:pagamentoLegado?.expiracao_pix||3600,descricao:pagamentoLegado?.descricao_pix||@json($atividade->nome)};
const pagamentoPixSicoob=document.getElementById('pagamento_pix_sicoob'),pagamentoPixExpiracao=document.getElementById('pagamento_pix_expiracao'),pagamentoPixDescricao=document.getElementById('pagamento_pix_descricao');
pagamentoPixSicoob.checked=!!pagamentoInicial.pix_sicoob;pagamentoPixExpiracao.value=pagamentoInicial.expiracao||3600;pagamentoPixDescricao.value=pagamentoInicial.descricao||@json($atividade->nome);
function atualizarPagamento(){const ativo=pagamentoPixSicoob.checked;document.querySelectorAll('.pagamento-pix-config').forEach(area=>area.hidden=!ativo);if(!pagamentoPixExpiracao.hasAttribute('disabled')){pagamentoPixExpiracao.disabled=!ativo;pagamentoPixDescricao.disabled=!ativo}window.refreshBuilderPayment?.()}
pagamentoPixSicoob.addEventListener('change',atualizarPagamento);atualizarPagamento();
...
function build(){const campos=window.readBuilderFields?.() ?? (initial.campos??[]).filter(campo=>campo.tipo!=='pagamento_pix');const conteudo=editorQuill?.root.innerHTML||'';const limitar=document.getElementById('limitar_inscricoes').checked,reserva=limitar&&document.getElementById('apos_encerrar_vagas').value==='lista_reserva',semLimite=reserva&&document.getElementById('lista_reserva_sem_limite').checked;return {limitar_inscricoes:limitar,limite_inscricoes:limitar?(Number(document.getElementById('limite_inscricoes').value)||null):null,apos_encerrar_vagas:reserva?'lista_reserva':'encerrar',limite_lista_reserva:reserva&&!semLimite?(Number(document.getElementById('limite_lista_reserva').value)||null):null,lista_reserva_sem_limite:semLimite,mostrar_vagas_restantes:limitar&&document.getElementById('mostrar_vagas_restantes').checked,registrar_presenca_qrcode:document.getElementById('registrar_presenca_qrcode').checked,pagamento_inscricao:{pix_sicoob:pagamentoPixSicoob.checked,expiracao:Number(pagamentoPixExpiracao.value)||3600,descricao:pagamentoPixDescricao.value.trim()||@json($atividade->nome)},mensagem_vagas_esgotadas:document.getElementById('mensagem_vagas_esgotadas').value,mensagem_ja_inscrito:document.getElementById('mensagem_ja_inscrito').value,mensagem_identificacao:document.getElementById('mensagem_identificacao').value,titulo:document.getElementById('titulo').value,subtitulo:document.getElementById('subtitulo').value,abertura:document.getElementById('abertura').value,fechamento:document.getElementById('fechamento').value,mensagem_antes:document.getElementById('mensagem_antes').value,mensagem_fechado:document.getElementById('mensagem_fechado').value,mensagem_sucesso:document.getElementById('mensagem_sucesso').value,editor:{exibir:document.getElementById('editor_exibir').checked,conteudo:conteudo==='<p><br></p>'?'':conteudo},grupos:groups,fieldsets,campos,criterios_vagas:window.readBuilderCriteria?.()??initial.criterios_vagas??[],rastreios:lerRastreios(),rows:[]}}
...
['titulo','subtitulo','abertura','fechamento','mensagem_antes','mensagem_fechado','mensagem_sucesso'].forEach(id=>document.getElementById(id).value=(id==='titulo'&&!String(initial[id]??'').trim()?@json($atividade->nome):(mensagensPadrao[id]&&!String(initial[id]??'').trim()?mensagensPadrao[id]:initial[id]??'')));document.getElementById('editor_exibir').checked=initial.editor?.exibir??!!initial.editor?.conteudo;
