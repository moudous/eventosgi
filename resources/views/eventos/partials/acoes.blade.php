...
@php($possuiDependencias = ($evento->atividades_count ?? 0) > 0 || ($evento->submissoes_count ?? 0) > 0)
...
@if($apagados)
    ...
    @endif
    @if($permissoes->permite('eventos.excluir_definitivamente') && ! $possuiDependencias)
...
@else
    ...
    @endif
    @if($permissoes->permite('eventos.excluir') && ! $possuiDependencias)
