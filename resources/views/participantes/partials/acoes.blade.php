...
{{-- Participante com certificado emitido nao e excluido: apagar o cadastro deixaria
     certificados apontando para ninguem. O controller recusa mesmo assim. --}}
@if($permissoes->permite('participantes.excluir') && $certificados['total'] === 0)<button class="btn btn-sm btn-outline-danger listagem-acao" data-delete-url="{{route('participantes.destroy',$params)}}" title="Excluir"><i class="bi bi-trash-fill"></i></button>@endif
</div>
