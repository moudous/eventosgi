...
<style>
body{background:{{ $submissao->corFundoPagina() }}}.submissao-publica{max-width:1120px;margin:0 auto;padding:28px 16px 60px}.submissao-hero{background:linear-gradient(135deg,#102a43,#176b87);color:#fff;border-radius:1rem;padding:30px}.submissao-hero h1{font-size:clamp(1.6rem,4vw,2.4rem)}.submissao-card{border:0;box-shadow:0 8px 26px rgba(16,42,67,.08)}.informacoes-publicas .ql-editor{font-size:1rem;min-height:0;padding:0}.informacoes-publicas .ql-editor img{height:auto;max-width:100%}.informacoes-publicas .ql-editor table{border-collapse:collapse;width:100%}.informacoes-publicas .ql-editor td{border:1px solid #dee2e6;padding:.5rem}.autor-linha{border:1px solid #dbe4ec;border-radius:.75rem;padding:1rem;background:#fff}.trabalho-opcao{border:1px solid #dbe4ec;border-radius:.75rem;padding:1rem}.modelo-editor.ql-container{height:480px}.conteudo-leitura{min-height:240px;background:#fff}.conteudo-leitura .ql-align-center{text-align:center}.conteudo-leitura .ql-align-right{text-align:right}.conteudo-leitura .ql-align-justify{text-align:justify}.isca{position:absolute!important;left:-10000px!important;width:1px!important;height:1px!important;overflow:hidden!important}.prazo-alerta{border-left:5px solid #0d6efd}.resultado-aprovado{border-left:6px solid #198754;background:#eaf7ef}.resultado-reprovado{border-left:6px solid #dc3545;background:#fdecee}
</style>
@endpush
@section('content')
@php
    ...
    $autorPrincipal = $trabalho && ($acesso['email'] ?? null) === $trabalho->inscricao->email;
    $podeEditar = $autorPrincipal && $submissao->aberta() && $trabalho->status === 'rascunho';
    $divulgarResultado = $trabalho?->avaliada() && $submissao->periodoEposterAberto();
@endphp
<div class="submissao-publica">
    @php($eventoVisual = $submissao->evento ?? new \App\Models\Evento)
    <header class="submissao-hero mb-4" style="background: {{ $eventoVisual->fundoFormulario('submissao') }}; color: {{ $eventoVisual->estiloFormulario('submissao')['cor_fonte'] }};"><div class="small text-uppercase opacity-75 fw-bold mb-2">{{ $submissao->evento?->nome }}</div><h1 class="mb-2">{{ $submissao->titulo }}</h1><div><strong>Submissão dos trabalhos:</strong> {{ $submissao->data_inicio?->format('d/m/Y H:i') }} até {{ $submissao->data_fim?->format('d/m/Y H:i') }}</div>@if($submissao->periodoEposterConfigurado())<div class="mt-1"><strong>Envio do e-pôster:</strong> {{ $submissao->eposter_data_inicio->format('d/m/Y H:i') }} até {{ $submissao->eposter_data_fim->format('d/m/Y H:i') }}</div>@endif @if($submissao->mostrar_link_evento && $submissao->evento?->ativo)<a href="{{ route('eventos.pagina.visualizar',$submissao->evento) }}" class="btn btn-light btn-sm mt-3"><i class="bi bi-arrow-left me-1"></i>Voltar para a página do evento</a>@endif</header>

    @if(trim(strip_tags((string) $submissao->informacoes)) !== '')<section class="card submissao-card informacoes-publicas mb-4"><div class="card-header bg-white"><h2 class="h5 mb-0">Informações</h2></div><div class="card-body p-4"><div class="ql-editor">{!! $submissao->informacoes !!}</div></div></section>@endif
    ...
    @else
        ...
        @if(!$acesso)
        <div class="card submissao-card mb-4"><div class="card-header bg-white"><h2 class="h5 mb-0">{{ $submissao->aberta() ? 'Inscreva-se e acesse seus trabalhos' : 'Acesse seus trabalhos' }}</h2></div><div class="card-body p-4">
            <p class="text-secondary">Se você já possui uma senha, entre para visualizar os trabalhos em que consta como autor ou coautor. Você também pode cadastrar um novo trabalho durante o período de submissão.</p>
        ...
        @if($trabalho && !$novo)
            @if($divulgarResultado && $trabalho->aprovada())
                <div class="alert resultado-aprovado"><h2 class="h4 text-success"><i class="bi bi-check-circle-fill me-2"></i>Trabalho aprovado</h2><p class="mb-1">Parabéns! Seu trabalho foi aprovado. Envie o arquivo da apresentação (e-pôster) em PDF dentro do prazo informado.</p>@if(filled($trabalho->nota))<div><strong>Nota:</strong> {{ $trabalho->nota }}</div>@endif</div>
                <div class="card submissao-card mb-4"><div class="card-header bg-white"><h2 class="h5 mb-0">Arquivo da apresentação — e-pôster</h2></div><div class="card-body p-4"><p class="text-secondary">Prazo de envio: <strong>{{ $submissao->eposter_data_inicio->format('d/m/Y H:i') }}</strong> até <strong>{{ $submissao->eposter_data_fim->format('d/m/Y H:i') }}</strong>.</p>@if($trabalho->temEposter())<div class="alert alert-success"><i class="bi bi-file-earmark-pdf me-1"></i>PDF enviado em {{ $trabalho->eposter_enviado_em?->format('d/m/Y H:i') }}. <a href="{{ route('submissoes.publicas.e-poster', [$submissao, $trabalho]) }}" target="_blank" rel="noopener" class="alert-link">Visualizar e-pôster</a></div>@endif @if($autorPrincipal)<form method="POST" action="{{ route('submissoes.publicas.enviar-e-poster', [$submissao, $trabalho]) }}" enctype="multipart/form-data">@csrf<label class="form-label fw-semibold" for="eposter">{{ $trabalho->temEposter() ? 'Substituir arquivo PDF' : 'Enviar arquivo PDF' }}</label><div class="input-group"><input type="file" class="form-control @error('eposter') is-invalid @enderror" id="eposter" name="eposter" accept="application/pdf,.pdf" required><button class="btn btn-success"><i class="bi bi-cloud-arrow-up me-1"></i>{{ $trabalho->temEposter() ? 'Substituir' : 'Enviar' }}</button></div><div class="form-text">Somente PDF, com no máximo 20 MB. O arquivo pode ser substituído até o fim do prazo.</div></form>@else<p class="mb-0 text-secondary">Apenas o primeiro autor pode enviar ou substituir o arquivo.</p>@endif</div></div>
            @elseif($divulgarResultado && $trabalho->situacao === 'reprovado')
                <div class="alert resultado-reprovado"><h2 class="h4 text-danger"><i class="bi bi-x-circle-fill me-2"></i>Trabalho reprovado</h2><p class="mb-1">Informamos que o trabalho não foi aprovado para apresentação. Não é necessário enviar e-pôster.</p>@if(filled($trabalho->nota))<div><strong>Nota:</strong> {{ $trabalho->nota }}</div>@endif</div>
            ...
            @else
                <div class="card submissao-card"><div class="card-header bg-white d-flex justify-content-between align-items-center gap-2"><h2 class="h5 mb-0">{{ $trabalho->titulo_trabalho }}</h2><div class="d-flex gap-2"><a class="btn btn-sm btn-outline-primary" href="{{ route('submissoes.publicas.exportar-documento', [$submissao, $trabalho]) }}"><i class="bi bi-file-earmark-word me-1"></i>Exportar documento</a><span class="badge text-bg-{{ $trabalho->avaliada()?'success':'info' }}">{{ $trabalho->avaliada() ? 'Avaliado' : ($trabalho->status === 'rascunho' ? 'Rascunho' : 'Submetido') }}</span></div></div><div class="card-body p-4"><dl class="row"><dt class="col-sm-3">E-mail do primeiro autor</dt><dd class="col-sm-9">{{ $trabalho->inscricao->email }}</dd>@if($submissao->mostrar_categoria_trabalho && $trabalho->categoriaTrabalhoRotulo())<dt class="col-sm-3">Categoria do trabalho</dt><dd class="col-sm-9">{{ $trabalho->categoriaTrabalhoRotulo() }}</dd>@endif<dt class="col-sm-3">Apresentação</dt><dd class="col-sm-9">{{ ucfirst($trabalho->apresentacao ?? 'presencial') }}</dd><dt class="col-sm-3">Apoio financeiro</dt><dd class="col-sm-9">{{ $trabalho->tem_apoio_financeiro ? ($trabalho->apoiador ?: 'Sim') : 'Não' }}</dd><dt class="col-sm-3">Aprovação do Comitê de Ética</dt><dd class="col-sm-9">{{ $trabalho->aprovacao_comite_etica ? 'Sim — Protocolo: '.($trabalho->protocolo_comite_etica ?: 'não informado') : 'Não' }}</dd></dl><div class="border rounded bg-light p-3 mb-4"><h3 class="h6 fw-bold">Autores</h3><div class="mb-3">@foreach($trabalho->autores as $autor){{ !$loop->first ? '; ' : '' }}{{ $autor->nome }}<sup>{{ $autor->numero ?? $autor->ordem }}</sup>@endforeach</div>@foreach($trabalho->autores as $autor)<div class="small"><sup>{{ $autor->numero ?? $autor->ordem }}</sup> {{ $autor->afiliacao ?: 'Afiliação não informada' }}</div>@endforeach</div>@if($submissao->mostrar_palavras_chave && $trabalho->palavras_chave)<h3 class="h6 fw-bold">Palavras Chave</h3><div class="d-flex flex-wrap gap-2 mb-4">@foreach(explode(',', $trabalho->palavras_chave) as $palavra)<span class="badge bg-light text-dark border border-secondary-subtle">{{ trim($palavra) }}</span>@endforeach</div>@endif<h3 class="h6 fw-bold">Resumo</h3><div class="border rounded p-4 conteudo-leitura">{!! $trabalho->conteudo ?: '<span class="text-secondary">Sem conteúdo.</span>' !!}</div></div></div>
            @endif
...
if(listaAutores){
    ...
    function atualizarVisualizacao(){
        ...
        autores.forEach((autor,indice)=>{
            ...
            const numero=document.createElement('sup');numero.textContent=String(indice+1);linha.append(numero,document.createTextNode(' '+(autor.afiliacao||'Filiação não informada')));
...
const modalRecuperacao = document.getElementById('recuperarSenhaModal');
modalRecuperacao.addEventListener('show.bs.modal', evento => {
    const cadastrar = evento.relatedTarget ? evento.relatedTarget.dataset.cadastrar === '1' : document.getElementById('cadastrar_credencial').value === '1';
    document.getElementById('cadastrar_credencial').value = cadastrar ? '1' : '0';
    modalRecuperacao.querySelector('.modal-title').textContent = cadastrar ? 'Cadastrar uma nova senha' : 'Recuperar senha';
    const email = document.getElementById('email_login')?.value.trim();
    if (email) document.getElementById('email_recuperacao').value = email;
});
@if($errors->has('senha_atual') || $errors->has('senha') || $errors->has('usar_na_atividade'))
    if(document.getElementById('alterarSenhaModal')) new bootstrap.Modal('#alterarSenhaModal').show();
@endif
